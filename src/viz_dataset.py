...
cross_tab_survey = cross_tab_survey.reindex(
    index=cross_tab.index, columns=era_order
).fillna(0)

# Create second figure with and axis and plot using the same colors and formatting
fig2, ax2 = plt.subplots(figsize=(12, 8))
cross_tab_survey.plot(
    kind="barh", stacked=True, color=colors_list, ax=ax2, legend=False
)

# Title and axis labels
ax2.set_title(
    f"Distribution of Item Types Across Eras (survey subset, n={len(subset)})", pad=20
)
ax2.set_ylabel("")
